...
public class NetworkClient implements KafkaClient {
    ...
    public boolean isReady(Node node, long now) {
        ...
        // priority
        // 1.不需要更新元数据信息, 2.生产者与broker已经建立TCP链接,ChannelReady
    ...
    public List<ClientResponse> poll(long timeout, long now) {
        ...
        try {
            // 触发真正的网络通讯，该方法中会通过收到调用NIO中的Selector#select方法，对管道的读写就绪事件进行处理，当写事件就绪后，就会将通道中的消息发送到远端的broker
        ...
        // 对请求结果进行唤醒，此时会将相应结果设置到KafkaProducer#send方法返回凭证中，从而唤醒客户端，完成一次完整到消息发送流程
        completeResponses(responses);
