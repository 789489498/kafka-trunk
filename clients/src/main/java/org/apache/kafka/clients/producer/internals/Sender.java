...
public class Sender implements Runnable {
    ...
    private long sendProducerData(long now) {
        ...
        // 根据已准备的分区，从缓存区中抽取发送的消息批次（ProducerBatch）抽取后的ProducerBatch将不再追加消息（就算还有剩余空间可用）
        ...
        // 从 inflightBatches 与 batches 中查找已过期的消息批次(ProducerBatch) 判断过期的标准是系统当前时间与ProducerBatch创建时间之差是否超过120s  delivery.timeout.ms  参数
        ...
        // 处理已超时的消息批次，通知该 ProducerBatch 消息发送失败，即通过设置
        ...
        // 7.发送请求到服务器，并处理服务端响应
        // 按brokerId 分别构建发送请求， 每一个broker会将多个ProducerBatch一起封装成一个请求进行发送，同一时间每一个与broker连接只会只能发送一个一个请求
        // 这里只是构建请求，最终会在  NetworkClient#send  方法，将该批数据设置到 NetworkClient 的待发送数据中，此时并没有真正用到网络请求
